...
// waitForLSNCatchup blocks until committed has acked all Postgres WAL
// up to the LSN that pg_current_wal_lsn() returns right now. This is
// the synchronization point that makes capture deterministic: once the
// slot's confirmed_flush_lsn >= the snapshot LSN, every CDC event
// generated by the mutation script is already in committed's Raft log.
//
// The committed ingestable acks per-commit via SendStandbyStatusUpdate
// (see postgres.go:357), so confirmed_flush_lsn moving past targetLSN
// means each COMMIT before that LSN has been fully processed by
// committed and written into a proposal.
func waitForLSNCatchup(ctx context.Context, conn *pgx.Conn, slotName string, timeout time.Duration) error {
	var targetLSN string
	if err := conn.QueryRow(ctx, "SELECT pg_current_wal_lsn()::text").Scan(&targetLSN); err != nil {
	...
	for time.Now().Before(deadline) {
		...
		if err == nil && confirmed != "" {
			// pg_lsn comparison via SQL — easier than parsing the
			// xx/yy hex format ourselves.
			var caughtUp bool
			err = conn.QueryRow(ctx,
				`SELECT $1::pg_lsn >= $2::pg_lsn`, confirmed, targetLSN,
			).Scan(&caughtUp)
			if err == nil && caughtUp {
				return nil
			}
	...
	return fmt.Errorf("slot %q did not catch up to LSN %s within %s", slotName, targetLSN, timeout)
}
