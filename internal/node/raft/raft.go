...
type raftNode struct {
	...
	commitC     chan<- *ctypes.AcceptedProposal // entries committed to log (k,v)
...
func NewRaftNode(id int, peers []string, join bool, dataDir string, getSnapshot func() ([]byte, error),
	...
	rc := &raftNode{
		...
		peers:       peers,
		join:        join,
		waldir:      filepath.Join(dataDir, fmt.Sprintf("raft-%d", id)),
		snapdir:     filepath.Join(dataDir, fmt.Sprintf("cluster-snap-%d", id)),
...
func (rc *raftNode) createAcceptedProposal(ent raftpb.Entry) (*ctypes.AcceptedProposal, error) {
...
func (rc *raftNode) publishEntries(ents []raftpb.Entry) bool {
	for i := range ents {
		...
		case raftpb.EntryNormal:
			...
			ap, err := rc.createAcceptedProposal(ents[i])
			if err != nil {
				// Log and ignore
