...
1. Hold the managed log mutex, validate arguments, and check handle health.
...
but unpublished files are orphans eligible for explicit `ReclaimOrphans`. If committed metadata
...
successful publication. A false value with an error does not prove that committed metadata
...
obsolete published segments and tails. Until then those files can retain the
...
and metadata commit failures exercise old/new recovery selection. These
