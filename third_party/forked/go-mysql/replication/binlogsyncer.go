...
type BinlogSyncerConfig struct {
	...
	// UseNumberForJSONDecimal emits a JSON-embedded DECIMAL leaf as an unquoted,
	// scale-preserving json.Number instead of a quoted string. committeddb fork
	// patch (see json_binary.go) so the CDC payload matches the snapshot path
	// byte-for-byte for opaque-decimal JSON leaves.
