...
func (e *RowsEvent) decodeJSONBinary(data []byte) ([]byte, error) {
	d := jsonBinaryDecoder{
		...
		useNumberForJSONDecimal:  e.useNumberForJSONDecimal,
...
func (d *jsonBinaryDecoder) decodeDecimal(data []byte) any {
	...
	// committeddb fork patch: emit a JSON-embedded DECIMAL leaf as an unquoted,
	// scale-preserving JSON number (json.Number) instead of a quoted string (the
	// useDecimal=false default) or a shopspring decimal (useDecimal=true, which
	// also trims trailing zeros). This makes the binlog (CDC) payload byte-for-byte
	// identical to the snapshot path, which renders the same decimal exact and
	// unquoted. See BinlogSyncerConfig.UseNumberForJSONDecimal. Upstream candidate.
	if d.useNumberForJSONDecimal {
		v, _, err := decodeDecimal(data[2:], precision, scale, false)
		d.err = err
		if s, ok := v.(string); ok {
